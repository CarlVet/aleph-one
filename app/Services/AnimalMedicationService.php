...
class AnimalMedicationService
{
    protected $projectId;

    private const DEFAULT_MEDICATION_OPTIONS = [
        'Amoxicillin',
    ...
    public function assign()
    {
        ...
        if ($this->isGuestMode()) {
            $animal_medications = AnimalMedication::with([
                'animals',
        ...
        $animals_existing = $animal_medications->unique('animals_id')->values();
        $medications_existing = $this->normalizeHistoricOptions(
            $animal_medications->pluck('medication_name')->all(),
            self::DEFAULT_MEDICATION_OPTIONS
        );
        $dosages_existing = $animal_medications->pluck('dosage')->unique()->values();
        ...
        return array_merge($baseCreateData, [
            ...
            'medications_existing' => $medications_existing,
            'dosages_existing' => $dosages_existing,
