...
class ParasiteSpeciesController extends Controller
{
    public function create()
    {
        return view('parasites.species.create');
    }

    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'name_scientific' => 'required|string|max:255|unique:parasite_species',
        ...
        ParasiteSpecies::create($request->all());

        // Check if this is a modal submission
        if ($request->ajax() || $request->wantsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Parasite species created successfully!',
            ]);
        }

        return redirect()->back()
            ->with('success', 'Parasite species created successfully!');
    }
    ...
    public function checkDuplicate(Request $request)
    {
        ...
        if (! in_array($field, ['name_scientific'], true)) {
        ...
        $candidates = ParasiteSpecies::query()
            ->pluck($field)
            ->filter()
