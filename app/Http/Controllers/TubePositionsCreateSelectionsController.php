...
use App\Services\TubeOriginSampleTypeResolver;
...
class TubePositionsCreateSelectionsController extends Controller
{
    ...
    public function humanTubes(Request $request): View
    {
        ...
        $paginationPath = route('bank.tubes.create.tubes.human');
    ...
    public function animalTubes(Request $request): View
    {
        ...
        $paginationPath = route('bank.tubes.create.tubes.animal');
    ...
    public function environmentTubes(Request $request): View
    {
        ...
        $paginationPath = route('bank.tubes.create.tubes.environment');
    ...
    public function parasiteTubes(Request $request): View
    {
        ...
        $paginationPath = route('bank.tubes.create.tubes.parasite');
    ...
    public function nucleicTubes(Request $request): View
    {
        ...
        $paginationPath = route('bank.tubes.create.tubes.nucleic');
    ...
    public function cultureTubes(Request $request): View
    {
        ...
        $paginationPath = route('bank.tubes.create.tubes.culture');
    ...
    public function poolTubes(Request $request): View
    {
        ...
        $paginationPath = route('bank.tubes.create.tubes.pool');
    ...
    private function tubeSearch(Request $request, string $contentType): JsonResponse
    {
        ...
        $resolver = app(TubeOriginSampleTypeResolver::class);
        ...
        $tubeCollection = new EloquentCollection($tubes->all());
        $resolver->hydrateTubes($tubeCollection);

        $results = $tubeCollection
            ->map(function (Tubes $tube) use ($resolver) {
                $label = $resolver->resolveForTube($tube);
    ...
    private function applyTubeSelectionSorting(Builder $query, string $contentType, int $sortCol, string $sortDir): void
    {
        ...
        // Ensure join-based sorting still returns Tube models
        $query->select('tubes.*');

        // Join related tables only when needed (non-tubes columns).
    ...
    private function tubesPaginator(string $contentType, string $pageName, array $filters, int $perPage, int $sortCol, string $sortDir)
    {
        ...
        $this->attachTubeSampleTypeLabels($paginator);

        return $paginator;
    }

    private function attachTubeSampleTypeLabels(LengthAwarePaginator $paginator): void
    {
        ...
        $resolver = app(TubeOriginSampleTypeResolver::class);
        $collection = new EloquentCollection($rows->all());
        $resolver->hydrateTubes($collection);

        $paginator->setCollection(
            $collection->map(function (Tubes $tube) use ($resolver): Tubes {
                $tube->setAttribute('tube_sample_type_label', $resolver->resolveForTube($tube));
    ...
    private function hydrateCultureTubeSelectionDetails(LengthAwarePaginator $paginator): void
    {
        ...
        (new EloquentCollection($cultures->all()))->loadMorph('cultures_content', [
            HumanSamples::class => ['sample_types', 'tubes'],
            AnimalSamples::class => ['sample_types', 'tubes'],
            EnvironmentSamples::class => ['environment_sample_types', 'tubes'],
            ParasiteSamples::class => ['parasite_sample_types', 'tubes'],
            NucleicAcids::class => ['nucleic_content', 'tubes'],
            Cultures::class => ['cultures_content', 'tubes'],
            Pools::class => ['pool_contents.samples', 'tubes'],
            Experiments::class => ['experiments_content', 'tubes'],
        ]);
    }
    ...
    private function applyNucleicTubeFilters(Builder $query, array $filters): void
    {
        ...
        $query->whereHasMorph('tubes_content', [NucleicAcids::class], function (Builder $naQuery) use ($filters) {
            ...
            if (! empty($filters[9])) {
                $this->applyMultiWordFilter($naQuery, (string) $filters[9], function (Builder $detailsQuery, string $value): void {
                    $detailsQuery
    ...
    private function applyPoolTubeFilters(Builder $query, array $filters): void
    {
        ...
        $query->whereHasMorph('tubes_content', [Pools::class], function (Builder $poolQuery) use ($filters) {
            ...
            if (! empty($filters[4])) {
                $poolQuery->whereHas('pool_contents', fn (Builder $q) => $q->where('samples_type', 'like', '%'.trim((string) $filters[4]).'%'));
            }
