<x-layout>
    <div class="mt-2 md:col-span-2 md:mt-0" x-data="{ registerMode: 'form' }">
        ...
        <div class="mx-auto mb-6 w-full max-w-3xl rounded-2xl border border-gray-200 bg-white px-6 py-4 shadow-sm">
            <div class="flex flex-col items-center justify-between gap-4 sm:flex-row">
                ...
                <div class="flex flex-wrap items-center justify-center gap-4 text-sm">
                    <label class="inline-flex items-center gap-2">
                        <input type="radio" name="animals_register_mode" value="form" x-model="registerMode"
                            class="h-4 w-4 text-blue-600 focus:ring-blue-500" />
                        <span class="text-gray-700">Form</span>
                    </label>
                    <label class="inline-flex items-center gap-2">
                        <input type="radio" name="animals_register_mode" value="import" x-model="registerMode"
                            class="h-4 w-4 text-blue-600 focus:ring-blue-500" />
        ...
        <div x-show="registerMode === 'import'" x-cloak class="mt-6">
            @if ($canEdit ?? false)
                <livewire:imports.animals-import />
            @else
                <div class="rounded-2xl border border-gray-200 bg-gray-50 px-6 py-4 text-sm text-gray-600">
                    <i class="fas fa-lock mr-2"></i>
                    Bulk import is disabled for viewers.
                </div>
            @endif
        </div>

        <div x-show="registerMode === 'form'" x-cloak>
            <x-forms.form method="POST" action="/animals" enctype="multipart/form-data">
                @csrf
                <div class="shadow-xl sm:overflow-hidden sm:rounded-xl bg-gradient-to-br from-white to-gray-50">
                    <div class="space-y-6 bg-white px-8 py-6 rounded-xl">
                        <div class="text-center mb-6">
                            ...
                            <p class="mt-2 text-sm text-gray-600">Fill in the details below to register one or more animals
                                with the same characteristics. Assign unique field labels to each animal.</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div class="mt-6 pt-4 border-t border-gray-100">
                                ...
                                <x-forms.field label="Upload Photo:" name="photo">
                                    <x-forms.photo-upload id="animal_photo" name="photo" label="Upload file" />
                        ...
                        <div class="space-y-6 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                            ...
                            <div id="field_label_assignment" class="space-y-4">
                                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                    ...
                                    <p class="text-xs text-gray-500 mb-3">Assign a unique field label to each animal
                                        (e.g., KNP001, AM002, etc.)</p>
            ...
            <x-table-modal id="animal_species_form_modal" title="Animals Registration Form"
                closeButtonId="animal_species_form_close_btn">
                @include('samples.animals.forms.form_animal_species')
            </x-table-modal>

            <x-table-modal id="animal_humans_modal" title="Human Registration Form"
                closeButtonId="animal_humans_close_btn">
                @include('samples.humans.forms.form_human')
            </x-table-modal>

            <x-table-modal id="animal_organization_modal" title="Organization Registration Form"
                closeButtonId="animal_organization_close_btn">
