<div>
    <x-forms.form id="animalSpeciesForm" method="POST" action="/animals/species" enctype="multipart/form-data">
        @csrf
            ...
            <div class="lg:col-span-2">
                <div class="space-y-6 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    <div class="text-center mb-6">
                        <h2 class="text-xl font-bold text-gray-800">Create New Animal Species</h2>
                    ...
                    <div class="space-y-4">
                        ...
                        <x-forms.field label="Common Name:" name="name_common">
                            <div class="relative">
                                <x-forms.text-input id="name_common" name="name_common" required
                                    ...
                                    placeholder="e.g., Lion, Elephant, Giraffe"></x-forms.text-input>
                                <div id="name_common_error" class="hidden mt-1 text-sm text-red-600"></div>
                                <div id="name_common_success" class="hidden mt-1 text-sm text-green-600">
                        ...
                        <x-forms.field label="Scientific Name:" name="name_scientific">
                            <div class="relative">
                                <x-forms.text-input id="name_scientific" name="name_scientific" required
                                    class="w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200"
                                    placeholder="e.g., Panthera leo, Loxodonta africana"></x-forms.text-input>
                        ...
                        <x-forms.field label="Genus:" name="genus">
                            <x-forms.text-input id="genus" name="genus"
                                class="w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200"
                                placeholder="e.g., Panthera, Loxodonta"></x-forms.text-input>
                        </x-forms.field>
                        ...
                        <x-forms.field label="Family:" name="family">
                            <x-forms.text-input id="family" name="family"
                                class="w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200"
                                placeholder="e.g., Felidae, Elephantidae"></x-forms.text-input>
                        </x-forms.field>
                        ...
                        <x-forms.field label="Order:" name="order">
                            <x-forms.text-input id="order" name="order"
                                class="w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200"
                                placeholder="e.g., Carnivora, Proboscidea"></x-forms.text-input>
                        </x-forms.field>
                        ...
                        <x-forms.field label="Class:" name="class">
                            <x-forms.text-input id="class" name="class"
                                class="w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200"
                                placeholder="e.g., Mammalia"></x-forms.text-input>
                        </x-forms.field>
                        ...
                        <x-forms.field label="Phylum:" name="phylum">
                            <x-forms.text-input id="phylum" name="phylum"
                                class="w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200"
                                placeholder="e.g., Chordata"></x-forms.text-input>
                        </x-forms.field>
                    </div>
                        ...
                        <div class="bg-white p-4 rounded-lg border border-blue-200">
                            <div class="flex items-start space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 mb-3">Verify the correct taxonomic classification using the IUCN Red List database!</p>
                            ...
                            <div class="bg-white p-4 rounded-lg border border-blue-200">
                                ...
                                <ul class="text-sm text-gray-600 space-y-2">
                                    <li class="flex items-start">
                                        ...
                                        <span><strong>Common Name:</strong> Use the most widely recognized common name</span>
                                    </li>
                                    <li class="flex items-start">
                                        <span class="w-2 h-2 bg-red-500 rounded-full mt-2 mr-2 flex-shrink-0"></span>
...
document.addEventListener('DOMContentLoaded', function() {
    const nameCommonInput = document.getElementById('name_common');
    const nameScientificInput = document.getElementById('name_scientific');
    const submitBtn = document.getElementById('submitBtn');

    let nameCommonTimeout;
    let nameScientificTimeout;
    let nameCommonStatus = 'empty';
    ...
    function checkDuplicate(field, value, errorElement, successElement) {
        if (!value.trim()) {
            ...
            if (field === 'name_common') {
                nameCommonStatus = 'empty';
            } else {
    ...
    function updateSubmitButton() {
        const hasBlockingError = nameCommonStatus === 'exact' || nameScientificStatus === 'exact';
        if (hasBlockingError) {
    ...
    nameCommonInput.addEventListener('input', function() {
        clearTimeout(nameCommonTimeout);
        const value = toTitleCaseWords(this.value).trim();
        const errorElement = document.getElementById('name_common_error');
        const successElement = document.getElementById('name_common_success');

        errorElement.classList.add('hidden');
        successElement.classList.add('hidden');

        if (value.length >= 2) {
            nameCommonTimeout = setTimeout(() => {
                checkDuplicate('name_common', value, errorElement, successElement);
            }, 500);
        }
    });
    ...
    nameCommonInput.addEventListener('blur', function() {
    ...
    document.getElementById('animalSpeciesForm').addEventListener('submit', function(e) {
        if (nameCommonStatus === 'exact' || nameScientificStatus === 'exact') {
            e.preventDefault();
