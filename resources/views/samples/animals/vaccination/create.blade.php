<x-layout>
    <div class="mt-2 md:col-span-2 md:mt-0">
        ...
        <form id="animalVaccinationForm" method="POST" action="/samples/animals/vaccination" enctype="multipart/form-data">
            @csrf
            <div class="shadow-xl sm:overflow-hidden sm:rounded-xl bg-gradient-to-br from-white to-gray-50">
                <div class="space-y-6 bg-white px-8 py-6 rounded-xl">
                    <div class="text-center mb-6">
                        <h2 class="text-2xl font-bold text-gray-800">Animal Vaccination Registration Form</h2>
                        <p class="mt-2 text-sm text-gray-600">Fill in the details below to register a new animal vaccination record
                        </p>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                            ...
                            <div class="pt-4 border-t border-gray-100">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <h2 class="text-lg font-semibold text-gray-800">Vaccination Information</h2>
                                </div>

                                <!-- Vaccine name input -->
                                <x-forms.field label="Vaccine name:" name="vaccine_name">
                                    <x-forms.select-input id="vaccine_name" name="vaccine_name[]" multiple required
                                        ...
                                        @foreach ($vaccines_existing as $vaccine)
                                ...
                                <!-- Vaccine type input -->
                                <x-forms.field label="Vaccine type:" name="vaccine_type">
                                    <x-forms.select-input id="vaccine_type" name="vaccine_type" required
                                        class="w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200">
                        ...
                        <div class="space-y-6 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                            <div class="flex items-center space-x-2">
                                ...
                                <h2 class="text-lg font-semibold text-gray-800">Vaccination Schedule</h2>
                            </div>

                            <!-- Date administered input -->
                            <x-forms.field label="Date administered:" name="date_administered">
                                <x-forms.date-input id="date_administered" name="date_administered" value="{{ now()->toDateString() }}"
                                    required
                                    class="w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200"></x-forms.date-input>
                            </x-forms.field>

                            <!-- Next due date input -->
                            <x-forms.field label="Next due date:" name="next_due_date">
                                <x-forms.date-input id="next_due_date" name="next_due_date"
                                    class="w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200"></x-forms.date-input>
                            </x-forms.field>

                            <!-- Administered by input -->
                            <x-forms.field label="Administered by:" name="administered_by">
                                <x-forms.select-input id="administered_by" name="administered_by"
                                    class="w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200">
                            ...
                            <x-forms.field label="Additional notes:" name="notes">
                                <x-forms.textarea id="notes" name="notes"
                                    class="w-full px-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200"
                                    placeholder="Enter any additional notes about the vaccination..."></x-forms.textarea>
                            </x-forms.field>
                        </div>
                    </div>
                </div>
        ...
        @push('scripts')
            ...
            <script>
                $(document).ready(function() {
                    // Initialize Selectize for vaccine name
                    $('#vaccine_name').selectize({
                        create: true,
                        sortField: 'text',
                        maxItems: null,
                        persist: false,
                    ...
                    // Initialize Selectize for vaccine type
                    $('#vaccine_type').selectize({
                        create: true,
                        sortField: 'text'
                    });

                    // Handle form submission with AJAX and SweetAlert
                    $('#animalVaccinationForm').on('submit', function(e) {
                        e.preventDefault();
