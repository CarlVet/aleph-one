        ...
        <div class="max-w-lg mx-auto mt-10 bg-white p-8 rounded shadow">
            @if ($profile_notice)
                <div class="mb-6 p-4 bg-blue-100 text-blue-800 rounded">
                    <strong>{{ $profile_notice }}</strong>
                </div>
            @endif
            <h2 class="text-xl font-bold mb-4">Project Invitation(s)</h2>
                ...
                @foreach ($invitations as $invitation)
                    <div class="mb-6 border rounded p-4 bg-gray-50">
                        ...
                        <form method="POST" action="{{ route('register.invitation.handle', $invitation->id) }}" class="flex flex-col gap-2">
                            ...
                            <input type="text" name="token" id="token_{{ $invitation->id }}" class="border rounded px-2 py-1">
