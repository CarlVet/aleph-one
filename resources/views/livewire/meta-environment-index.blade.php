<div class="text-center mt-2">
    ...
    <div class="mt-8 border border-gray-200 rounded-xl shadow-xl bg-white">
        <div class="flex flex-col items-center w-full p-4">
            ...
            <h3 class="text-lg text-gray-600 mb-4">from environment samples</h3>
            ...
            @if ($showBulkActions)
                <div class="mt-3 flex items-center gap-3">
                    ...
                    <span class="text-sm text-gray-600">{{ count(array_filter($selectedMetaEnvironments ?? [])) }} selected</span>
                </div>
            @endif
        </div>

        @include('livewire.partials.index-per-page-toolbar', ['paginator' => $meta_environments])


        <div class="index-table-container overflow-x-auto">
        <table id="meta_environments_table" class="index-data-table min-w-full divide-y divide-gray-200 {{ ($showBulkActions ?? false) ? 'has-bulk-select' : '' }}">
            <thead>
                <tr class="bg-gradient-to-r from-gray-50 to-gray-100">
                    ...
                    @endif
                    @if ($isGuestMode)
                        <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200"><x-sort-button field="project" :active="$sortField" :direction="$sortDirection">Project</x-sort-button></th>
                    @endif
                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200"><x-sort-button field="study" :active="$sortField" :direction="$sortDirection">Study</x-sort-button></th>
                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200"><x-sort-button field="environment_type" :active="$sortField" :direction="$sortDirection">Sample Type</x-sort-button></th>
                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200"><x-sort-button field="location" :active="$sortField" :direction="$sortDirection">Location</x-sort-button></th>
                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200"><x-sort-button field="country" :active="$sortField" :direction="$sortDirection">Country</x-sort-button></th>
                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200"><x-sort-button field="date_sampling" :active="$sortField" :direction="$sortDirection">Date Sampling</x-sort-button></th>
                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200"><x-sort-button field="pathogen" :active="$sortField" :direction="$sortDirection">Pathogen</x-sort-button></th>
                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200"><x-sort-button field="technique" :active="$sortField" :direction="$sortDirection">Technique</x-sort-button></th>
                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200"><x-sort-button field="tested_n" :active="$sortField" :direction="$sortDirection">Tested N</x-sort-button></th>
                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200"><x-sort-button field="pos_n" :active="$sortField" :direction="$sortDirection">Positive N</x-sort-button></th>
                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200">Risk Factors</th>
                    ...
                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider border-b border-gray-200"><x-sort-button field="scientist" :active="$sortField" :direction="$sortDirection">Scientist</x-sort-button></th>
            ...
            <thead class="bg-gradient-to-r from-gray-100 to-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3">
                        <input type="text" wire:model.live.debounce.300ms="environmentTypeFilter"
                            class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm transition-all duration-200"
                            placeholder="Filter">
                    </th>
                    <th class="px-6 py-3">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($meta_environments as $meta)
                    <tr wire:key="{{ $meta->id }}" class="hover:bg-gray-50 transition-all duration-200 ease-in-out transform hover:scale-[1.01]">
                        @if ($showBulkActions)
                            <td class="px-6 py-4 whitespace-nowrap">
                                <input type="checkbox" wire:model.live="selectedMetaEnvironments.{{ $meta->id }}"
                        ...
                        @if ($isGuestMode)
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-gradient-to-r from-purple-100 to-purple-200 text-purple-800 shadow-sm">N/A</span>
                                @endif
                            </td>
                        @endif
                        <td class="px-6 py-4 whitespace-nowrap min-w-[18rem]">
                            @if ($isEditing && ! $isGuestMode && $canEdit)
                                <select
                                    class="w-full min-w-[18rem] rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-800 shadow-sm focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-500"
                            ...
                            @else
                                <a href="/studies/{{ $meta->studies->id ?? '#' }}" class="text-blue-600 hover:text-blue-800 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap min-w-[18rem]">
                            @if ($isEditing && ! $isGuestMode && $canEdit)
                                <select
                                    class="w-full min-w-[18rem] rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-800 shadow-sm focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-500"
                                    x-data="{ original: @js((int) ($meta->environment_sample_types_id ?? 0)) }"
                                    x-on:change.stop.prevent="
                                        ...
                                        original = $el.value;
                                        $wire.updateField({{ $meta->id }}, 'environment_sample_types_id', $el.value);
                                    "
                                >
                                    @foreach (($environmentSampleTypesOptions ?? []) as $opt)
                                        <option value="{{ $opt->id }}" @selected((int) ($meta->environment_sample_types_id ?? 0) === (int) $opt->id)>
                                            {{ $opt->name }}
                                        </option>
                                    @endforeach
                                </select>
                            @else
                                <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-gradient-to-r from-green-100 to-green-200 text-green-800 shadow-sm">
                                    {{ $meta->environment_sample_types->name ?? 'N/A' }}
                                </span>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap min-w-[18rem]">
                            @if ($isEditing && ! $isGuestMode && $canEdit)
                                <input type="text"
                                    ...
                                    "
                                    class="w-full min-w-[18rem] px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200"
                        ...
                        <td class="px-6 py-4 whitespace-nowrap min-w-[18rem]">
                            ...
                            @else
                                <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-gradient-to-r from-red-100 to-red-200 text-red-800 shadow-sm">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap min-w-[10rem]">
                            @if ($isEditing && ! $isGuestMode && $canEdit)
                                <input type="number" min="0"
                                    ...
                                    "
                                    class="w-full min-w-[10rem] px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent shadow-sm transition-all duration-200"
                        ...
                        <td class="px-6 py-4 whitespace-nowrap index-people-cell">
                            <div class="flex items-center space-x-3">
                                ...
                                <a href="/profile/{{ $meta->people->id }}" class="text-blue-600 hover:text-blue-800 transition-colors duration-200">
        ...
        @include('livewire.partials.index-pagination-bar', ['paginator' => $meta_environments])
    </div>
</div>
