...
wire:ignore.self>
    <div class="px-4 py-6 sm:px-0">
        ...
        @else
            <!-- Header Section -->
            <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-t-xl shadow-lg">
                <div class="px-6 py-8">
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            <div class="flex items-center space-x-3 mb-4">
                                <div class="bg-white/20 rounded-lg w-20 h-20 flex items-center justify-center">
                                    <i class="fas fa-paw text-white text-[40px] group-hover:rotate-12 transition-transform duration-300"></i>
                                  </div>
                                <div>
                                    <h1 class="text-3xl font-bold text-white">Animal Sample Details</h1>
                                    <p class="text-orange-100 text-lg">Code: {{ $animalSample->code }}</p>
                                    @if(optional($animalSample->subProjectAssignment?->subProject)->code)
                                        <span class="mt-2 inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-indigo-100 text-indigo-800">
                                            Sub-project: {{ $animalSample->subProjectAssignment->subProject->code }}
                                        </span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="flex items-center space-x-4">
                                <span class="text-orange-100 text-sm">
                                    {{ $animalSample->sample_types->name ?? 'N/A' }} • {{ ucfirst($animalSample->animals->animal_species->name_common) }}
                                </span>
                                @if(!$canEdit)
                        ...
                        <div class="flex space-x-3">
                            ...
                            @if($canEdit)
                                <button wire:click="deleteAnimalSample"
                                        wire:confirm="Are you sure you want to delete this animal sample? This action cannot be undone."
            ...
            <div class="bg-white shadow-lg rounded-b-xl">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 p-8">
                    ...
                    <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="bg-gray-50 rounded-xl p-6">
                            <div class="flex items-center mb-6">
                                <div class="bg-yellow-100 p-2 rounded-lg mr-3">
                                    <i class="fas fa-paw text-lg text-yellow-600 group-hover:rotate-12 transition-transform duration-300"></i>
                                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 mb-1">Animal</dt>
                                    <dd class="text-sm text-gray-900">
                                        <a href="/animals/{{ $animalSample->animals->code }}" class="text-blue-600 hover:text-blue-800 font-medium hover:underline">
                                            {{ $animalSample->animals->code }} - {{ $animalSample->animals->animal_species->name_common ?? 'N/A' }}
                                        </a>
                                    </dd>
                                </div>
                                ...
                                <div>
                                    ...
                                    <dd class="text-sm text-gray-900">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                            {{ $animalSample->animals->sex === 'Male' ? 'bg-blue-100 text-blue-800' : 
                                               ($animalSample->animals->sex === 'Female' ? 'bg-pink-100 text-pink-800' : 
                                               'bg-gray-100 text-gray-800') }}">
                                            {{ $animalSample->animals->sex ?? 'N/A' }}
                                        </span>
                                    </dd>
                                </div>
                                ...
                                <div>
                                    ...
                                    <dd class="text-sm text-gray-900 font-medium">
                                        <div class="inline-edit" wire:key="date_received_{{ $animalSample->id }}">
                                            <span class="{{ $canEdit ? 'editable-text cursor-pointer hover:bg-gray-100 px-2 py-1 rounded transition-colors' : '' }}"
                                                  @if($canEdit) wire:click="startEdit('date_received')" @endif
                                                  x-show="!editing.date_received">
                                                {{ $animalSample->date_received ? \Carbon\Carbon::parse($animalSample->date_received)->format('Y-m-d') : 'N/A' }}
                                            </span>
                                            @if($canEdit)
                                                <div x-show="editing.date_received" class="inline-flex items-center space-x-2">
                                                    <input type="date"
                                                           wire:model="editingValues.date_received"
                                                           ...
                                                           x-ref="date_received_input"
                                                           x-init="$nextTick(() => $refs.date_received_input.focus())">
                                                    <button wire:click="saveEdit('date_received')"
                                                    ...
                                                    <button wire:click="cancelEdit('date_received')"
                                ...
                                <div>
                                    ...
                                    <dd class="text-sm text-gray-900 font-medium">
                                        <div class="inline-edit" wire:key="area_{{ $animalSample->id }}">
                                            ...
                                            @if($canEdit)
                                                <div x-show="editing.area" class="inline-flex items-center space-x-2">
                                                    <input type="text" 
                                                           ...
                                                           class="px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                                           placeholder="{{ $animalSample->area ?? 'Enter area' }}"
                                                           x-ref="area_input"
                                ...
                                <div>
                                    ...
                                    <dd class="text-sm text-gray-900 font-medium">
                                        <div class="inline-edit" wire:key="coordinates_{{ $animalSample->id }}">
                                            <span class="{{ $canEdit ? 'editable-text cursor-pointer hover:bg-gray-100 px-2 py-1 rounded transition-colors' : '' }}" 
                                                  @if($canEdit) wire:click="startEdit('latitude')" @endif
                                                  x-show="!editing.latitude && !editing.longitude">
                                                @if($animalSample->latitude && $animalSample->longitude)
                                                    {{ $animalSample->latitude }}, {{ $animalSample->longitude }}
                                                @else
                                                    N/A
                                                @endif
                                            </span>
                                ...
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 mb-1">Storage Location</dt>
                                    <dd class="text-sm text-gray-900 font-medium">
                                        {{ $animalSample->locations->name ?? 'N/A' }}
                                    </dd>
                                </div>

                                <div>
                                    <dt class="text-sm font-medium text-gray-500 mb-1">Storage State</dt>
                                    <dd class="text-sm text-gray-900">
                                        <div class="inline-edit" wire:key="storage_state_{{ $animalSample->id }}">
                                            <span class="{{ $canEdit ? 'editable-text cursor-pointer hover:bg-gray-100 px-2 py-1 rounded transition-colors' : '' }}" 
                                                  @if($canEdit) wire:click="startEdit('storage_state')" @endif
                                                  x-show="!editing.storage_state">
                                                <span class="inline-flex items-center text-sm font-medium">
                                                    {{ ucfirst(str_replace('_', ' ', $animalSample->storage_state)) ?? 'N/A' }}
                                                </span>
                                            </span>
                                            @if($canEdit)
                                                <div x-show="editing.storage_state" class="inline-flex items-center space-x-2">
                                                    <div class="relative min-w-[150px]">
                                                        <input type="text" wire:model="editingValues.storage_state" 
                                                            x-ref="storage_state_input"
                                                            x-init="$nextTick(() => $refs.storage_state_input.focus())"
                                                            value="{{ $animalSample->storage_state }}"
                                                        ...
                                                        <datalist id="storage-state-list">
                                                            @foreach($existingStorageStates as $state)
                                                                <option value="{{ $state }}">
                                                                    {{ $state }}
                                                                </option>
                                                            @endforeach
                                                    ...
                                                    <button wire:click="saveEdit('storage_state')" 
                                                            class="w-6 h-6 bg-green-500 hover:bg-green-600 text-white rounded-full flex items-center justify-center transition-colors duration-200">
                                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                                        </svg>
                                                    </button>
                                                    <button wire:click="cancelEdit('storage_state')" 
                                                            class="w-6 h-6 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center transition-colors duration-200">
                                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                                        </svg>
                                ...
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 mb-1">Processed</dt>
                                    <dd class="text-sm text-gray-900">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                            {{ $animalSample->processed ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                            {{ $animalSample->processed ? 'Yes' : 'No' }}
                                        </span>
                                    </dd>
                                </div>
                        ...
                        @if($animalSample->nucleic_acids->count() > 0)
                        <div class="bg-gray-50 rounded-xl p-6 mb-6" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center justify-between w-full p-4 bg-white rounded-lg border hover:bg-gray-50 transition-colors duration-200">
                                <div class="flex items-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900">Nucleic Acids ({{ $animalSample->nucleic_acids->count() }})</h3>
                                </div>
                                        ...
                                        <table class="min-w-full divide-y divide-gray-200">
                                            <thead class="bg-gray-50">
                                                <tr>
                                                    ...
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date Extracted</th>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                                                </tr>
                                            </thead>
                                            <tbody class="bg-white divide-y divide-gray-200">
                                                @foreach($animalSample->nucleic_acids as $na)
                                                <tr class="hover:bg-gray-50">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                        {{ $na->date_extracted ? \Carbon\Carbon::parse($na->date_extracted)->format('M d, Y') : 'N/A' }}
                                                    </td>
                                                    <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        @if($animalSample->microplastics->count() > 0)
                        <div class="bg-gray-50 rounded-xl p-6 mb-6" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center justify-between w-full p-4 bg-white rounded-lg border hover:bg-gray-50 transition-colors duration-200">
                                <div class="flex items-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900">Microplastics ({{ $animalSample->microplastics->count() }})</h3>
                                </div>
                                <svg class="w-5 h-5 text-gray-500 transform transition-transform duration-200" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                                </svg>
                                ...
                                <div class="bg-white rounded-lg border overflow-hidden">
                                    <div class="overflow-x-auto">
                                        <table class="min-w-full divide-y divide-gray-200">
                                            ...
                                            <tbody class="bg-white divide-y divide-gray-200">
                                                @foreach($animalSample->microplastics as $microplastic)
                                                <tr class="hover:bg-gray-50">
                                                    <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <!-- Cultures Section -->
                        @if($animalSample->cultures->count() > 0)
                        <div class="bg-gray-50 rounded-xl p-6 mb-6" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center justify-between w-full p-4 bg-white rounded-lg border hover:bg-gray-50 transition-colors duration-200">
                                <div class="flex items-center">
                                    <div class="bg-orange-100 p-2 rounded-lg mr-3">
                                        <i class="fa-solid fa-bacteria text-lg text-orange-600"></i>
                                    </div>
                                    <h3 class="text-lg font-medium text-gray-900">Cultures ({{ $animalSample->cultures->count() }})</h3>
                                </div>
                                <svg class="w-5 h-5 text-gray-500 transform transition-transform duration-200" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                                </svg>
                                        ...
                                        <table class="min-w-full divide-y divide-gray-200">
                                            <thead class="bg-gray-50">
                                                <tr>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Code</th>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Type</th>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date Cultured</th>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Created By</th>
                                                </tr>
                                            </thead>
                                            <tbody class="bg-white divide-y divide-gray-200">
                                                @foreach($animalSample->cultures as $culture)
                                                <tr class="hover:bg-gray-50">
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                        <div class="flex items-center space-x-2">
                                                            ...
                                                            <a href="/profile/{{ $culture->people->id ?? '' }}" class="text-blue-600 hover:text-blue-800 hover:underline">
                                                                {{ $culture->people ? ($culture->people->title ?? '') . ' ' . ($culture->people->first_name ?? '') . ' ' . ($culture->people->last_name ?? '') : 'N/A' }}
                                                            </a>
                                                        </div>
                                                    </td>
                                                </tr>
                        ...
                        <!-- Pools Section -->
                        @if($animalSample->pools->count() > 0)
                        <div class="bg-gray-50 rounded-xl p-6 mb-6" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center justify-between w-full p-4 bg-white rounded-lg border hover:bg-gray-50 transition-colors duration-200">
                                <div class="flex items-center">
                                    <div class="bg-cyan-100 p-2 rounded-lg mr-3">
                                        <i class="fa-solid fa-layer-group text-lg text-cyan-600"></i>
                                    </div>
                                    <h3 class="text-lg font-medium text-gray-900">Pools ({{ $animalSample->pools->count() }})</h3>
                                </div>
                                <svg class="w-5 h-5 text-gray-500 transform transition-transform duration-200" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                                </svg>
                                        ...
                                        <table class="min-w-full divide-y divide-gray-200">
                                            <thead class="bg-gray-50">
                                                <tr>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Code</th>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Type</th>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date Pooled</th>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Created By</th>
                                                </tr>
                                            </thead>
                                            <tbody class="bg-white divide-y divide-gray-200">
                                                @foreach($animalSample->pools as $pool)
                                                <tr class="hover:bg-gray-50">
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                        <a href="/samples/pools/{{ $pool->code }}" class="text-blue-600 hover:text-blue-800 font-medium hover:underline">
                                                            {{ $pool->code }}
                                                        </a>
                                                    </td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                        {{ $pool->date_pooled ? \Carbon\Carbon::parse($pool->date_pooled)->format('M d, Y') : 'N/A' }}
                                                    </td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                        <div class="flex items-center space-x-2">
                                                            ...
                                                            <a href="/profile/{{ $pool->people->id ?? '' }}" class="text-blue-600 hover:text-blue-800 hover:underline">
                                                                {{ $pool->people ? ($pool->people->title ?? '') . ' ' . ($pool->people->first_name ?? '') . ' ' . ($pool->people->last_name ?? '') : 'N/A' }}
                                                            </a>
                                                        </div>
                                                    </td>
                                                </tr>
                        ...
                        <div class="bg-gray-50 rounded-xl p-6 mb-6" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center justify-between w-full p-4 bg-white rounded-lg border hover:bg-gray-50 transition-colors duration-200">
                                <div class="flex items-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900">Tubes ({{ $animalSample->tubes->count() }})</h3>
                                </div>
                                <svg class="w-5 h-5 text-gray-500 transform transition-transform duration-200" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                                </svg>
                                        ...
                                        <table class="min-w-full divide-y divide-gray-200">
                                            <thead class="bg-gray-50">
                                                <tr>
                                                    ...
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Preservant</th>
                                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Location</th>
                                                </tr>
                                            </thead>
                                            <tbody class="bg-white divide-y divide-gray-200">
                                                @foreach($animalSample->tubes as $tube)
                                                <tr class="hover:bg-gray-50">
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                        @if($tube->tube_positions && $tube->tube_positions->count() > 0)
                        ...
                        @if($animalSample->nucleic_acids->count() === 0 && $animalSample->experiments->count() === 0 && $animalSample->cultures->count() === 0 && $animalSample->pools->count() === 0 && $animalSample->tubes->count() === 0)
                            <div class="bg-gray-50 rounded-xl p-6">
                        ...
                        <div class="space-y-4">
                                ...
                                <dd class="text-sm text-gray-900">
                                    <div class="flex items-center space-x-3 bg-white p-3 rounded-lg border">
                                        <x-people-logo :person="$animalSample->people" width="40" />
                                        <div>
                                            <a href="/profile/{{ $animalSample->people->id }}"
                                                class="text-blue-600 hover:text-blue-800 font-medium hover:underline">
                                                {{ $animalSample->people ? ($animalSample->people->title ?? '') . ' ' . ($animalSample->people->first_name ?? '') . ' ' . ($animalSample->people->last_name ?? '') : 'N/A' }}
                                            </a>
                                            @if ($animalSample->people && $animalSample->people->email)
                                                <p class="text-xs text-gray-500">{{ $animalSample->people->email }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </dd>
