<div>
        ...
        <div class="bg-gradient-to-r from-purple-100 to-blue-100 border border-purple-300 rounded-lg p-4 mb-6">
            <div class="flex items-center justify-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-purple-600">You are viewing public nucleic acids from all projects</p>
                </div>
                <a href="/my-projects"
    ...
    <div class="text-center mb-4">
        <h1 class="text-3xl font-bold text-gray-800 mb-2">Nucleic Acids Dashboard</h1>
        <p class="text-gray-600">Overview of all nucleic acid extractions and their outcomes</p>
    </div>
        ...
        <div x-show="open" x-collapse>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select wire:model.live.debounce.300ms="nucleicTypeFilter"
                        ...
                        @foreach ($allNucleicTypes as $type)
                ...
                @if ($sourceTypeFilter === 'animal')
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Animal species</label>
                        <select wire:model.live.debounce.300ms="animalSpeciesFilter"
                            class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm transition-all duration-200">
                ...
                @if ($sourceTypeFilter === 'parasite')
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Parasite origin type</label>
                        <select wire:model.live.debounce.300ms="parasiteOriginTypeFilter"
                            class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm transition-all duration-200">
                            <option value="all">All origins</option>
                            @foreach ($availableParasiteOriginTypes as $key => $label)
                                <option value="{{ $key }}">{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>

                    @if ($parasiteOriginTypeFilter === 'animal')
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Origin animal species</label>
                            <select wire:model.live.debounce.300ms="parasiteOriginAnimalSpeciesFilter"
                                class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm transition-all duration-200">
                                <option value="">All species</option>
                                @foreach ($allAnimalSpecies as $species)
                                    <option value="{{ $species }}">{{ $species }}</option>
                ...
                @if (in_array($sourceTypeFilter, ['parasite', 'culture', 'pool', 'nucleic'], true))
                ...
                <div>
                    ...
                    <select wire:model.live.debounce.300ms="protocolFilter"
                        class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm transition-all duration-200">
                ...
                <!-- Extracted By Filter -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Extracted By</label>
                    <select wire:model.live.debounce.300ms="extractedByFilter"
                        class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm transition-all duration-200">
                ...
                <!-- Date Range Filter -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Extraction Date Range</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100 transform transition-all duration-300 hover:scale-105 cursor-pointer"
            onclick="openModal('nucleicAcidsModal')">
        ...
        <!-- Parasite Samples -->
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100 transform transition-all duration-300 hover:scale-105 cursor-pointer"
            onclick="openModal('parasiteSamplesModal')">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600">Parasite Samples</p>
                    <p class="text-3xl font-bold text-amber-600">{{ $descriptive_stats['parasite_samples'] }}</p>
                </div>
                <div class="p-3 bg-amber-100 rounded-full">
                    <i class="fas fa-bug text-2xl text-amber-600"></i>
                </div>
            </div>
        </div>
    ...
    <div class="dashboard-box bg-white rounded-xl shadow-lg p-6 border border-gray-100 mb-4">
        <!-- Extraction Timeline Chart -->
        <div>
            <h4 class="text-md font-semibold text-gray-700 mb-4">Extraction Timeline</h4>
    ...
    <!-- Modal for All Nucleic Acids -->
    <div id="nucleicAcidsModal"
        class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
        <div class="bg-white rounded-lg p-6 max-w-6xl w-full mx-4 max-h-[80vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-800">All Nucleic Acids in Filtered Dataset</h3>
                <button onclick="closeModal('nucleicAcidsModal')" class="text-gray-500 hover:text-gray-700">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>
            <div id="nucleicAcidsModalContent" data-modal-content class="text-sm text-gray-500">Loading…</div>
        </div>
    </div>
    ...
    <div id="humanSamplesModal"
        ...
        <div class="bg-white rounded-lg p-6 max-w-6xl w-full mx-4 max-h-[80vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-800">Human Nucleic Acid Samples</h3>
    ...
    <div id="animalSamplesModal"
        ...
        <div class="bg-white rounded-lg p-6 max-w-6xl w-full mx-4 max-h-[80vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-800">Animal Nucleic Acid Samples</h3>
    ...
    <div id="environmentSamplesModal"
        ...
        <div class="bg-white rounded-lg p-6 max-w-6xl w-full mx-4 max-h-[80vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-800">Environment Nucleic Acid Samples</h3>
    ...
    <!-- Modal for Parasite Samples -->
    <div id="parasiteSamplesModal"
        class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
        <div class="bg-white rounded-lg p-6 max-w-6xl w-full mx-4 max-h-[80vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-800">Parasite Nucleic Acid Samples</h3>
                <button onclick="closeModal('parasiteSamplesModal')" class="text-gray-500 hover:text-gray-700">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>
            <div id="parasiteSamplesModalContent" data-modal-content class="text-sm text-gray-500">Loading…</div>
        </div>
    </div>

    <!-- Modal for Culture Samples -->
    <div id="cultureSamplesModal"
        class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
        <div class="bg-white rounded-lg p-6 max-w-6xl w-full mx-4 max-h-[80vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-800">Culture Nucleic Acid Samples</h3>
    ...
    <!-- Modal for Pool Samples -->
    <div id="poolSamplesModal"
        class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
        <div class="bg-white rounded-lg p-6 max-w-6xl w-full mx-4 max-h-[80vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-800">Pool Nucleic Acid Samples</h3>
    ...
    <script>
        const timelineData = @json($descriptive_stats['extraction_timeline']);
        const nucleicAcidsBySource = @json($nucleicAcidsBySource);
        const nucleicAcidsByType = @json($nucleicAcidsByType);
        const nucleicAcidsByProtocol = @json($nucleicAcidsByProtocol);
        const nucleicAcidsByLaboratory = @json($nucleicAcidsByLaboratory);
        const nucleicAcidsByExtractedBy = @json($nucleicAcidsByExtractedBy ?? []);
        ...
        window.nucleicAcidsBySource = nucleicAcidsBySource;
        window.nucleicAcidsByType = nucleicAcidsByType;
        window.nucleicAcidsByProtocol = nucleicAcidsByProtocol;
        window.nucleicAcidsByLaboratory = nucleicAcidsByLaboratory;
        window.nucleicAcidsByExtractedBy = nucleicAcidsByExtractedBy;
        ...
        function openModal(modalId) {
            ...
            window.loadNucleicDashboardModal?.(modalId);
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const modals = ['nucleicAcidsModal', 'humanSamplesModal', 'animalSamplesModal',
                'environmentSamplesModal', 'parasiteSamplesModal', 'cultureSamplesModal', 'poolSamplesModal'
            ];
    ...
    @push('scripts')
        ...
        <script src="/js/show-nucleic-acids.js?v={{ filemtime(public_path('js/show-nucleic-acids.js')) }}"></script>
    @endpush
