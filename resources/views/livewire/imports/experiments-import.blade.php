        ...
        <div class="flex items-start justify-between gap-6">
            <div>
                <h3 class="text-lg font-semibold text-gray-800 flex items-center gap-2">
                    ...
                    Bulk import experiments (CSV)
                </h3>
                <p class="mt-1 text-sm text-gray-600">
                    Upload a CSV, review row validations, then import.
                </p>
            </div>
        </div>
    </div>
        ...
        <div x-show="templateOpen" x-cloak
            ...
            <div class="flex h-[95vh] w-full max-w-[98vw] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/10">
                <div class="flex items-start justify-between gap-6 border-b border-gray-200 bg-gray-50 px-6 py-4">
                    <div>
                        <h4 class="text-lg font-semibold text-gray-900">Experiments import template (CSV)</h4>
                        <p class="mt-1 text-sm text-gray-600">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 pr-10 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Protocol</th>
                                <th class="px-4 py-3 pl-10 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Pathogen</th>
                                <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Outcome</th>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Tested by</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 bg-white">
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-900 min-w-[12rem]">
                                        <div class="inline-flex items-center gap-1">
                                            @if (!empty($row['tube_exists']))
                                                <i class="fa-solid fa-link text-blue-700"></i>
                                                <span>{{ $row['selected_tube_alias_code'] ?: '-' }}</span>
                                            @else
                                                <i class="fa-solid fa-circle-xmark text-red-700"></i>
                                                <span>{{ $row['tube_alias'] ?: '-' }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 pl-10 text-sm text-gray-900 min-w-[20rem]">
                                        <div class="inline-flex items-center gap-1">
                                            @if (!empty($row['protocol_exists']))
                                                <i class="fa-solid fa-link text-blue-700"></i>
                                            @elseif (!empty($row['field_warnings']['protocol_name']))
                                            ...
                                            <input type="text"
                                                ...
                                                onblur="this.value = window.alephNormalizeNameStyle(this.value); this.dispatchEvent(new Event('input', { bubbles: true }))"
                                                wire:input="applySuggestedValue({{ (int) ($row['row_number'] ?? 0) }}, 'protocol_name', $event.target.value)" />
                                        </div>
                                        <div class="mt-1 grid grid-cols-1 gap-1">
                                            @if (empty($row['protocol_exists']))
                                                <input type="text"
                                                    wire:model.live="rowOverrides.{{ (int) ($row['row_number'] ?? 0) }}.technique_type"
                                                    placeholder="technique_type for new protocol"
                                                    class="w-full min-w-[14rem] rounded border border-gray-200 px-2 py-1 text-xs" />
                                                <select
                                                    wire:model.live="rowOverrides.{{ (int) ($row['row_number'] ?? 0) }}.technique_category"
                                                    class="w-full min-w-[14rem] rounded border border-gray-200 px-2 py-1 text-xs">
                                                    <option value="">Select technique category</option>
                                                    @foreach ($allowedTechniqueCategories as $category)
                                                        <option value="{{ $category }}">{{ $category }}</option>
                                                    @endforeach
                                                </select>
                                            @endif
                                        </div>
                                        @if (!empty($row['field_warnings']['protocol_name']))
                                            @foreach (($row['field_warnings']['protocol_name']['options'] ?? []) as $option)
                                                <button type="button"
                                                    wire:click="applySuggestedValue({{ (int) ($row['row_number'] ?? 0) }}, 'protocol_name', @js($option))"
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-900">
                                        <div class="inline-flex items-center gap-1">
                                            @if (!empty($row['pathogen_exists']))
                                                <i class="fa-solid fa-link text-blue-700"></i>
                                            @elseif (!empty($row['field_warnings']['pathogen']))
                                                <i class="fa-solid fa-triangle-exclamation text-yellow-700"></i>
                                            @else
                                                <i class="fa-solid fa-plus text-green-700"></i>
                                            @endif
                                            <span>{{ $row['pathogen'] ?: '-' }}</span>
                                        </div>
                                        @if (empty($row['pathogen_exists']))
                                            <div class="mt-1">
                                                <button type="button"
                                                    onclick="(function(){const m=document.getElementById('pathogen_import_modal'); if(m){m.classList.remove('hidden'); m.classList.add('flex');}})()"
                                                    class="inline-flex items-center gap-1 text-[11px] text-blue-800 underline hover:text-blue-600">
                                                    <i class="fa-solid fa-plus"></i>
                                                    Register new pathogen
                                                </button>
                                            </div>
                                        @endif
                                        @if (!empty($row['field_warnings']['pathogen']))
                                            @foreach (($row['field_warnings']['pathogen']['options'] ?? []) as $option)
                                                <button type="button"
                                                    wire:click="applySuggestedValue({{ (int) ($row['row_number'] ?? 0) }}, 'pathogen', @js($option))"
                                                    class="mt-1 mr-2 text-[11px] text-yellow-900 underline hover:text-yellow-700">
                                                    Use "{{ $option }}"
                                                </button>
                                            @endforeach
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-900">
                                        <div>{{ $row['outcome_type'] ?: 'Qualitative only' }}</div>
                                        <div class="text-xs text-gray-700">{{ $row['outcome_discrete'] ?: '-' }}</div>
                                        <div class="text-xs text-gray-500">{{ $row['outcome_quant'] ?: '-' }}</div>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-900">{{ $row['date_tested'] ?: '-' }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-900">
                                        <div class="inline-flex items-center gap-1">
                                            @if (!empty($row['laboratory_exists']))
                                                <i class="fa-solid fa-link text-blue-700"></i>
                                            @elseif (!empty($row['field_warnings']['laboratory']))
                                                <i class="fa-solid fa-triangle-exclamation text-yellow-700"></i>
                                            @else
                                                <i class="fa-solid fa-plus text-green-700"></i>
                                            @endif
                                            <input type="text"
                                                ...
                                                onblur="this.value = window.alephNormalizeNameStyle(this.value); this.dispatchEvent(new Event('input', { bubbles: true }))"
                                                wire:input="applySuggestedValue({{ (int) ($row['row_number'] ?? 0) }}, 'laboratory', $event.target.value)" />
                                        </div>
                                        @if (empty($row['laboratory_exists']))
                                            <input type="text"
                                                wire:model.live="rowOverrides.{{ (int) ($row['row_number'] ?? 0) }}.laboratory_country"
                                                placeholder="laboratory_country for new lab"
                                                class="mt-1 w-full min-w-[13rem] rounded border border-gray-200 px-2 py-1 text-xs" />
                                        @endif
                                        @if (!empty($row['field_warnings']['laboratory']))
                                            @foreach (($row['field_warnings']['laboratory']['options'] ?? []) as $option)
                                                <button type="button"
                                                    wire:click="applySuggestedValue({{ (int) ($row['row_number'] ?? 0) }}, 'laboratory', @js($option))"
                                                    class="mt-1 mr-2 text-[11px] text-yellow-900 underline hover:text-yellow-700">
                                                    Use "{{ $option }}"
                                                </button>
                                            @endforeach
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-900">
                                        <div class="inline-flex items-center gap-1">
                                            @if (!empty($row['tested_by_exists']))
                                                <i class="fa-solid fa-link text-blue-700"></i>
                                            @elseif (!empty($row['field_warnings']['tested_by']))
                                                <i class="fa-solid fa-triangle-exclamation text-yellow-700"></i>
                                            @else
                                                <i class="fa-solid fa-plus text-green-700"></i>
                                            @endif
                                            <span>{{ $row['tested_by'] ?: '-' }}</span>
                                        </div>
                                        @if (!empty($row['field_warnings']['tested_by']))
                                            @foreach (($row['field_warnings']['tested_by']['options'] ?? []) as $option)
                                                <button type="button"
                                                    wire:click="applySuggestedValue({{ (int) ($row['row_number'] ?? 0) }}, 'tested_by', @js($option))"
                                                    class="mt-1 mr-2 text-[11px] text-yellow-900 underline hover:text-yellow-700">
                                                    Use "{{ $option }}"
                                                </button>
                                            @endforeach
