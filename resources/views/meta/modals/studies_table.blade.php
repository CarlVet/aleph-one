@php
    /** @var \Illuminate\Support\Collection|\Illuminate\Contracts\Pagination\LengthAwarePaginator|null $studies */
        ...
        <table id="studies_table" class="w-full">
            ...
            <tbody class="bg-white divide-y divide-gray-100">
                @foreach ($studies as $study)
                    <tr class="hover:bg-green-50 transition-all duration-200 ease-in-out group" wire:key="{{ $study->id }}">
                        <td class="sticky left-0 z-10 bg-white group-hover:bg-green-50 px-6 py-4 border-r border-gray-100">
                            <div class="flex items-center">
                                <input type="radio" name="study_selection" class="select-study h-5 w-5 text-green-600 focus:ring-green-500 border-gray-300 transition-all duration-200 hover:scale-110" 
                                       value="{{ $study->id }}" 
                                       data-text="{{ $study->ref_key }}">
                            </div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            @if($study->doi)
                                <a href="https://doi.org/{{ $study->doi }}" target="_blank" class="text-sm text-blue-600 hover:text-blue-800 hover:underline">
                                    {{ $study->doi }}
                                </a>
                            @else
